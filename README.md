...
The contract implements **per-transfer auditing** and supports **multiple auditors per token**: the sender attaches an auditor-readable copy of each transfer amount (two u32-limb decryption handles per receiver, per auditor key) derived from the same range-proven commitments the receiver gets, plus one short zero-knowledge proof per receiver (folding the amount's two u32 limbs) that ties every auditor's handle to the receiver's own range-proven commitments. Balances stay encrypted only under the user's own key, so the same account key can be reused across tokens. See [Auditor Support in Confidential Transfers](AUDITORS.md) for the design and the alternatives considered.
...
Auditing is per transfer, not per account. For each [`TransferEvent`](move/sources/events.move), the auditor calls `decryptTransferAmount(event)`: it finds the held key that matches one of the event's `auditor_pks`, reads that key's handle set (the event carries one per auditor key), takes the two u32-limb commitments straight from the event's `encrypted_amount_receiver`, pairs each with the matching handle, and BSGS-decrypts to recover that transfer's amount. An auditor can hold several keys, so keeping the rotated-out keys lets it read transfers made both before and after a key rotation. The auditor sees each transfer's amount, sender, and receiver — but never a user's viewing key or account balance.
