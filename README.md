...
- **[`ts-sdk/`](ts-sdk/)** -- TypeScript SDKs that mirror the Move modules:
  ...
  - **[`ContraAuditor`](ts-sdk/src/auditor.ts)** -- auditor SDK: decrypts a transfer's amount from its `TransferEvent` using one of the token's auditor keys.
...
The contract implements **per-transfer auditing** with a single auditor key per token (rotatable, with a grace window): the sender attaches an auditor-readable copy of each transfer amount (two u32-limb decryption handles per receiver) derived from the same range-proven commitments the receiver gets, plus one short zero-knowledge proof per u32 limb (two per receiver) that ties the auditor's handle to the receiver's own range-proven commitment. Balances stay encrypted only under the user's own key, so the same account key can be reused across tokens. See [Auditor Support in Confidential Transfers](AUDITORS.md) for the design and the alternatives considered.

### Onboarding flow

1. **Generate an auditor keypair off-chain.** Each auditor generates a Twisted ElGamal keypair `(sk, pk)` over Ristretto255 and hands the **public key** to the token issuer.
2. **Issuer sets the on-chain auditor keys.** The issuer calls [`update_auditors`](move/sources/contra.move) with two key vectors: `current_pks` (the keys tried first on a transfer; empty disables auditing going forward) and `previous_pks` (also accepted, for a grace window). The two need not be the same length. No per-user escrow. Grace is caller-driven, with no expiration epoch: to change the set with a grace window set `current_pks` to the new keys (which may be fewer, or empty to disable) while `previous_pks` still holds the outgoing keys, so in-flight transfers built against the old keys still verify; end the grace by setting `previous_pks` equal to `current_pks`.
...
Auditing is per transfer, not per account. For each [`TransferEvent`](move/sources/events.move), the auditor calls `decryptTransferAmount(event)`: it uses the held key that matches the event's `auditor_pk` (`none` when auditing is disabled), reads that auditor's two handles from `auditor_decryption_handles`, folds the event's four u16 commitments (`encrypted_amount_receiver`) into two u32-limb commitments, pairs each with the matching handle, and BSGS-decrypts to recover that transfer's amount. A token has one auditor key at a time, but an auditor can hold several keys, so keeping the rotated-out keys lets it read transfers made both before and after a key rotation. The auditor sees each transfer's amount, sender, and receiver — but never a user's viewing key or account balance.
