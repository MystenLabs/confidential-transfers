...
Auditing is per transfer, not per account. For each [`TransferEvent`](move/sources/events.move), the auditor calls `decryptTransferAmount(event)`: it uses the held key that matches the event's `auditor_pks` (a length-0-or-1 vector), reads its handles from `auditor_decryption_handles` at the same index, takes the two u32-limb commitments straight from the event's `encrypted_amount_receiver`, pairs each with the matching handle, and BSGS-decrypts to recover that transfer's amount. A token has one auditor key at a time, but an auditor can hold several keys, so keeping the rotated-out keys lets it read transfers made both before and after a key rotation. The auditor sees each transfer's amount, sender, and receiver — but never a user's viewing key or account balance.
