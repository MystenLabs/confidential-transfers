...
- **One auditor key with a grace window.** The token stores two key vectors, `current_pks` (tried first on a transfer) and `previous_pks` (also accepted), **each holding at most one key** (asserted on `new`/`update`). A transfer verifies against `current_pks`, else against `previous_pks`, so a transfer built against the outgoing key just before a rotation still validates. There is no expiration epoch; the issuer drives the grace explicitly with one `update(current_pks, previous_pks)` call: rotate with `update(new, old)`, end the grace with `update(new, new)`, and disable auditing going forward with an empty `current_pks`.

- **Cheap per-transfer overhead, no auditor range proofs.** A twisted-ElGamal ciphertext `c = r·g + m·h` is key-independent, so the receiver's four range-proven u16-limb commitments double as the auditor's commitments: the chain folds them into two u32-limb commitments (`C_0 + 2^16 C_1`, `C_2 + 2^16 C_3`) for free, and the sender attaches only two decryption handles per receiver amount — `D̃_l = ρ̃_l · pk_auditor`, one per u32 limb `l`. No separate auditor commitments or range proofs.

- **A DDH proof instead of an ElGamal proof.** The receiver's *own* u32 decryption handle `ρ̃_l · pk_recv` (part of the transfer it already range/consistency-proves) pins each limb's blinding `ρ̃_l` to the commitment, so the sender does not re-prove well-formedness. It proves only that the auditor handle re-keys that same `ρ̃_l` — one **DDH** per u32 limb over the bases `[pk_recv, pk_auditor]` mapping to the images `[ρ̃_l · pk_recv, ρ̃_l · pk_auditor]`, anchored by the receiver's handle at the first base. So each transfer carries two small DDH proofs per receiver.

  (The two per-limb DDHs share the same bases and differ only in their blinding, so they could later be folded into one batched DDH per receiver — the DDH analog of the batched `ElGamalProof` — to halve the proof count; a `TODO` in `auditors::verify_under` notes this. It cannot fold across receivers, since each receiver contributes a distinct `pk_recv` base.)

- **Offboarding is clean.** Because the auditor holds no user viewing keys, removing the auditor key immediately ends its visibility into future transfers, with no user-driven viewing-key rotation required (unlike Option 2).

Auditors read amounts from `TransferEvent`s (the auditor's two handles plus the two u32-limb commitments in `encrypted_amount_receiver`); reading is stateful (balances are reconstructed from the transfer graph), and history before the auditor's key was installed requires the previous keys, which the issuer key-derivation scheme above can supply.
