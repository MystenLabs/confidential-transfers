 ...
 * key. A `TransferEvent` names that key in `auditor_pks` (a length-0-or-1 vector) and carries its
 * `[lo, hi]` handles at the same index in `auditor_decryption_handles` (both empty when auditing is
 * disabled). The event's `encrypted_amount_receiver` already carries the two u32-limb commitments
 * (`Ǎ_0, Ǎ_1`), so this pairs each with the matching handle to form a twisted ElGamal ciphertext and
 * recovers the amount.
 *
 ...
 * transfer's `auditor_pks`.
 */
export class ContraAuditor {
	#tokenType: string;
	// Keyed by the hex of the public key's compressed bytes, so a transfer's `auditor_pks` (the same
	...
	/**
	 * Register an auditor private key so transfers naming its public key (`pk = sk * G`) in `auditor_pks`
	 ...
	 * `auditor_pks` entry, pairs each of the event's two u32-limb commitments
	 ...
	 *   disabled for it — empty `auditor_pks`).
	 * @throws {@link AuditorKeyNotHeldError} if this auditor doesn't hold the event's `auditor_pks` key;
	...
	decryptTransferAmount(event: DecodedTransferEvent): bigint | null {
		// `auditor_pks` / `auditor_decryption_handles` are length 0 (auditing disabled) or 1.
		const auditorPk = event.auditor_pks[0];
		if (auditorPk === undefined) return null;
		...
		const pair = event.auditor_decryption_handles[0];
