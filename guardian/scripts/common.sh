...
hex_vector_literal() {
    ...
    if [ -z "$hex" ] || ! [[ "$hex" =~ ^[0-9a-fA-F]+$ ]] || [ $(( ${#hex} % 2 )) -ne 0 ]; then
        echo "error: $name must be a non-empty, even-length hexadecimal string" >&2
        exit 1
    fi
...
prepare_fleet_operation() {
    ...
    require_commands aws jq pulumi sui
    check_operations_repo
...
remove_guardian_key() {
    ...
    sui client call \
        ...
        --module guardian \
        --function remove_enclave \
        --type-args "$TOKEN_TYPE" \
        --args "$GUARDIAN_ID" "$key_index" \
