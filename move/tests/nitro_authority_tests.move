...
/// Unit tests for the canonical Nitro authority module.
#[test_only]
module contra::nitro_authority_tests;
...
/// A dummy token type; `NitroAuthority<T>` is only exercised standalone here.
...
/// A NitroAuthority with PCRs `00/01/02` operated by `ALICE`.
fun test_nitro_authority(): nitro_authority::NitroAuthority<TestCurrency> {
    let mut ctx = alice_ctx();
    nitro_authority::new_for_testing(
        x"00",
        x"01",
        x"02",
        ALICE,
...
#[test]
fun nitro_authority_updates() {
    let mut reg = test_nitro_authority();
...
fun min_version_bump_prunes_stale_keys() {
    ...
    *(&mut key_c[0]) = 0xc1;
    ...
    // register key_a
    let slot_fixture = reg.register_enclave_for_testing(ENCLAVE_PK, ENC_PK);
    let slot_a = reg.register_enclave_for_testing(key_a, ENC_PK);

    // update to key_b
    reg.update(&management_cap, x"10", x"11", x"12", 0, ALICE);
    assert!(reg.contains_nitro_authority_enclave_key(slot_fixture));
    assert!(reg.contains_nitro_authority_enclave_key(slot_a));
    let slot_b = reg.register_enclave_for_testing(key_b, ENC_PK);

    // update to key_c
    reg.update(&management_cap, x"20", x"21", x"22", 0, ALICE);
    let slot_c = reg.register_enclave_for_testing(key_c, ENC_PK);
    ...
    assert!(!reg.contains_nitro_authority_enclave_key(slot_fixture));
    assert!(!reg.contains_nitro_authority_enclave_key(slot_a));
    assert!(!reg.contains_nitro_authority_enclave_key(slot_b));
    assert!(reg.contains_nitro_authority_enclave_key(slot_c));
...
#[test, expected_failure(abort_code = ::contra::nitro_authority::EInvalidMinVersion)]
...
fun operator_can_remove_key() {
    let mut reg = test_nitro_authority();
    let index = reg.register_enclave_for_testing(ENCLAVE_PK, ENC_PK);
    assert!(reg.contains_nitro_authority_enclave_key(index));
...
#[test, expected_failure(abort_code = ::contra::nitro_authority::ENotOperator)]
...
#[test]
fun rotated_operator_can_manage_nitro_authority() {
    let mut reg = test_nitro_authority();
    let management_cap = new_management_cap();
    let index = reg.register_enclave_for_testing(ENCLAVE_PK, ENC_PK);
...
#[test, expected_failure(abort_code = ::contra::nitro_authority::ENotOperator)]
fun former_operator_cannot_manage_nitro_authority() {
    let mut reg = test_nitro_authority();
    let management_cap = new_management_cap();
    reg.update(&management_cap, x"00", x"01", x"02", 0, BOB);
...
fun registration_uses_lowest_emptied_slot() {
    let mut reg = test_nitro_authority();
    assert_eq!(reg.register_enclave_for_testing(ENCLAVE_PK, ENC_PK), 0);
    assert_eq!(reg.register_enclave_for_testing(ENC_PK, ENC_PK), 1);
    let mut ctx = alice_ctx();
    reg.remove_enclave(0, &mut ctx);
    assert_eq!(reg.register_enclave_for_testing(ENCLAVE_PK, ENC_PK), 0);
    assert!(reg.contains_nitro_authority_enclave_key(1));
    unit_test::destroy(reg);
}

#[test, expected_failure(abort_code = ::contra::nitro_authority::ETooManyNitroAuthorityEnclaveKeys)]
fun registration_rejects_65th_key() {
    let mut reg = test_nitro_authority();
    64u8.do!(|i| {
        let mut signing_pk = ENC_PK;
        *(&mut signing_pk[0]) = i;
        reg.register_enclave_for_testing(signing_pk, ENC_PK);
    });
    reg.register_enclave_for_testing(ENCLAVE_PK, ENC_PK);
    abort
}
...
fun distinct_limb_binding_digest_regression() {
    ...
    assert_eq!(
        authority::digest_for_testing(&payload),
        x"92b3b166c324b7e048a4002d702051b7220bdc7a4f05b500f45dcfe92ce33e2e",
    );
}

/// Rust counterpart: `move_types::tests::nitro_authority_request_matches_move`.
#[test]
fun nitro_authority_request_serde() {
    let payload = authority::transfer_binding(
        public_key(ristretto255::g_generator()),
        vector[public_key(ristretto255::g_generator())],
        zero_amount(),
    ...
    assert_eq!(
        std::bcs::to_bytes(
            &nitro_authority::new_nitro_authority_request_for_testing(
                authority::digest_for_testing(&payload),
            ),
        ),
        x"002023c5d517304afc26e1651eae6ed659d8109f14db6565420135ce9ce1ada495a3",
    );
}
