...
- **client.ts**: `ContraClient` -- builds Move call transactions for register, wrap, unwrap, transfer, and account management. Transfers attach per-transfer auditor data (`buildAuditorData`: two u32-limb handles per receiver + one batched `ElGamalNizk`) when `getAuditor(tokenType)` reports an enabled key. Key rotation follows the Move Option A model: `setAccountKey` (O(1)), `rekeyToken` / `tryRekeyToken` (per token; the latter soft-fails), and the `rotateKey` convenience (set-key + merge + re-key in one PTB).
