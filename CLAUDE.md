...
- **guardian.move**: Optional TEE second factor for transfers and unwraps (design: `guardian/README.md`). `GuardianPolicy` (held as `Option<GuardianPolicy>` on `ConfidentialToken` — `Some` enables it) tracks the expected enclave image `Pcrs`, a `version`/`min_version` pair for lazy key invalidation, a single operator address, an operator-updatable `url`, and the registered `GuardianEnclaveKey`s (VecMap keyed by ed25519 `signing_pk`). Registration verifies a `sui::nitro_attestation` document against the policy PCRs and slices both keys out of its `user_data` (`signing_pk || enc_pk`, 32 bytes each). When enabled, `batched_transfer` and `unwrap` require a `GuardianApproval`: an enclave signature over the BCS `RequestPayload` enum (`Transfer` / `Unwrap`, whose variant tag domain-separates them), rebuilt on-chain from the transaction's own arguments — binding the sender's current encrypted balance makes a signature single-use, so there is no timestamp or TTL. The issuer (`ManagementCap`) sets/unsets and updates the policy; the operator registers and removes keys and updates `url`. Both entry-point sets live in `contra.move`; events in `events.move`. The off-chain half is the Rust workspace in `guardian/`.
...
- **core/**: wire types byte-compatible with `contra::guardian`, the plaintext arithmetic checks, and response signing. No Nitro, no networking, so it is unit-testable off-enclave. Values are fastcrypto types (`RistrettoPoint`, `Ciphertext`, `Ed25519PublicKey`) everywhere they are computed on; only `RequestPayload`, the one struct the chain rebuilds and BCS-encodes itself, is expressed in the Move-shaped `MoveElement` / `MoveCiphertext`, because Move's `Element<G>` wraps a `vector<u8>` and so carries a length prefix that fastcrypto's bare point encoding omits. Conversion is one-way, in `checks::construct_payload`, which validates before constructing and whose `VerifiedRequestPayload` result is constructible nowhere else — only a validated request can reach the signer. fastcrypto's encoding cannot be changed to match, since `EllipticCurveMultisetHash` digests a serialized `RistrettoPoint` and is Sui's `GlobalStateHash`. `tests` in `types.rs` pin the bytes against the Move golden vectors.
- **enclave/**: the binary — key generation at boot, `/attestation`, `/health` + `/ready` (the proxy's readiness gate), and `/approve` for HPKE-sealed requests. `--features non-enclave-dev` stubs the NSM attestation call so it runs outside an enclave.
