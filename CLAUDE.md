...
- **auditor.ts**: `ContraAuditor` -- per-transfer auditor. Holds the token's auditor private key and `decryptTransferAmount(encryptedAmountReceiver, auditorHandles)` recovers a transfer's amount from a `TransferEvent`: it regroups the receiver's four u16 limbs into the two u32-limb commitments (mirroring `encrypted_amount::auditor_commitments`), pairs each with the matching handle, and BSGS-decrypts. The auditor never learns a user's viewing key.
