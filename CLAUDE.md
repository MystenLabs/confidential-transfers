...
- **balance.move**: `EncryptedBalance<T>` — one account's holdings of token `T`, all under a single key: the key itself (`pk`), the spendable `active` balance, the `pending` encrypted deposits, and `public_balance`, the wrapped-but-unmerged public deposits. `active`/`pending` are `BoundedEncryptedAmount` (an `EncryptedAmount` plus a count of merged u16-bounded values that bounds limb growth), value crosses the module boundary as one of two abilityless claims — `EncryptedCoin<T>` for encrypted value and `PublicCoin<T>` for public value, the latter redeemed against `Pool<T>` by `contra.move`, which owns all pool I/O — while `public_balance` tracks the wrapped-but-unmerged total in plaintext. `EncryptedBalance<T>` and `EncryptedCoin<T>` are `phantom`-parameterized by the token type, mirroring `sui::balance::Balance<T>` / `sui::coin::Coin<T>`, so different token types can't be mixed (the inner `BoundedEncryptedAmount` needs no tag — it never leaves its parent). The module is **self-contained**: every operation is checked against the balance's own `pk` (`deposit_encrypted` aborts `EInvalidPublicKey` unless the coin is encrypted under it, and `try_replace_active` — the single point where the active balance changes — re-checks the key of any verified amount handed back to it) and every proof is verified against a Fiat-Shamir DST the module derives itself from the caller's `session_id`, the `DST_DDH` / `DST_ELGAMAL` / `DST_RANGE_PROOF_16` / `DST_BATCH_DDH` protocol ids living here; `contra.move` keeps only what isn't value (authorization, freezing, events). The API is deposits (`deposit_public` / `deposit_encrypted`, both capacity-checked against `EBalanceFull`), consolidation (`merge_deposits`), verification (`verify_amount` for a single amount under the balance's own key, `verify_transfer_amounts` for a batch's receiver amounts, sender new balance and transfer total), withdrawals of those verified amounts (`try_withdraw_public`, returning a `Coin<T>` redeemed from the pool; `try_withdraw_encrypted`, returning receiver-keyed `EncryptedCoin`s — verification is a separate step so the caller can run the auditor check against the verified receiver amounts before the balance is touched, and `verify_transfer_amounts` hands back one abilityless `VerifiedTransfer<T>` that only `try_withdraw_encrypted` can consume, so the transfer total can never reach the withdrawal paired with receiver amounts it was not summed from), re-statement (`try_update_active`), key rotation (`try_rekey`, which aborts `EPendingDepositsMustBeMerged` unless `pending` is empty and swaps `pk` on success), and the `TreasuryCap`-gated issuer override `overwrite_unchecked`. `verify_transfer_amounts` reconstructs the transfer total's commitment from the receiver amounts (the sender and receiver commitments are identical), so only its single sender-keyed decryption handle is supplied by the caller and proven well-formed together with the new balance in one folded ElGamal proof; a DDH balance proof then shows the balance drops by exactly that total. The sender therefore never sends per-limb sender-keyed handles.
