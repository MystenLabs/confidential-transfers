...
- **decode.move**: `entry` deserializers that build the composite crypto types (`Encryption`, `EncryptedAmount`, `PublicKey`, `DdhProof`, `ElGamalProof`, and vectors of each) from flat `vector<vector<u8>>` element encodings in one Move call. They exist because a PTB cannot `makeMoveVec` over elements that are themselves `moveCall` results — that fails on chain with a type mismatch — so the ts-sdk (`helpers.ts`'s `build*` functions) flattens every point/scalar to 32 bytes and calls a single decoder, which re-validates each part via `g_from_bytes` / `scalar_from_bytes`.
