...
- **contra.move**: Main contract — `TokenRegistry`, `AccountRegistry`, `Account`, `ConfidentialToken<T>`, `ManagementCap<T>`. Orchestrates register, wrap (public→confidential), unwrap (confidential→public), and transfer operations. Supports a permissioned mode where `register`/`wrap`/`unwrap` can be gated behind an issuer policy. Uses Sui dynamic fields for account state. Each token's balance is keyed under a per-token `TokenAccount.pk`, chosen explicitly at `register<T>(account, auth, pk)` and rotated explicitly via `rekey_token<T>(account, auth, new_pk, ...)` — per-token keys are independent of each other and of the account key. `Account.default_pk` is an **optional** default key (`Option<Element<G>>`): the only thing it drives is `register_with_default_pk`. `set_default_pk` sets or clears it (`Option`, O(1), touches no balance). Key rotation is lazy: `rekey_token<T>` requires the token's `pending` empty (merge first) and re-keys `active` from `TokenAccount.pk` to the supplied `new_pk` under a per-token DDH (`balance::try_set_public_key`); the optimistic sibling `try_rekey_token<T>` soft-fails (emits `TryTokenRekeyFailedEvent`, returns `false`, leaves the token stale) instead of aborting, so `set_default_pk` + several re-keys can ride in one PTB without pausing. Deposits land under the receiver's `TokenAccount.pk` (senders read the per-token key), so `active`/`pending` never mix keys and a stale token keeps receiving under its old key until re-keyed. Deposits never auto-register: `wrap` and `add_to_batch` abort with `EReceiverNotRegistered` if the receiver has no `TokenAccount<T>`. For a permissionless-register token whose owner set `Account.default_pk`, anyone can create one on the receiver's behalf up front with `register_with_default_pk<T>(account, ct)` (keyed at `Account.default_pk`; aborts `EDefaultPkNotSet` if unset, `ERegistrationNotPermissionless` if the token's `register` op is permissioned) — the SDK inserts this automatically before a `wrap`/`transfer` to an as-yet-unregistered receiver. Deposits (`wrap` / `add_to_batch`) are gated on the per-`TokenAccount` `accepts_deposits`. Auditing is **per-transfer**: when a token has an auditor key, `batched_transfer` takes an `auditors::AuditorPackage` (one `U32LimbHandles` — two u32-limb auditor decryption handles — per receiver plus one batched `ElGamalProof`), checked in `auditors::verify_transfer` against the current or (in grace) previous auditor key at `ctx.epoch()`.
- **auditors.move**: `Auditor` — a token's single auditor public key with a rotation grace window (`current_pk`, `previous_pk`, `previous_expiration_epoch`) — and `AuditorPackage`, the per-transfer data a sender attaches (one `U32LimbHandles` per receiver + one batched `ElGamalProof`). `verify_transfer(auditor, receiver_amounts, auditor_package, epoch, dst)` owns the whole per-transfer check: it derives the u32-limb encryptions from `receiver_amounts`, enforces the presence policy, and accepts the proof under the current key or the previous key while `epoch <= previous_expiration_epoch`, returning the per-receiver handles + the verifying key. Auditor data is required when a current key is set, forbidden when fully off (no current key and no previous key in grace), and optional during a disable grace window (`current_pk == none` but the previous key is still within `previous_expiration_epoch`) — data, if attached, is verified under the previous key so in-flight transfers stay auditable (`accepts_at`).
...
- **nizk.move**: Fiat-Shamir NIZKs over Ristretto255 — `DdhProof` (shared-witness DDH over a vector of base/image pairs: the classic Chaum-Pedersen tuple is the two-pair case, used for balance/equality proofs; per-token key rotation is the five-pair case, one `w` re-keying the token's public key and all four limb decryption handles at once) and `ElGamalProof` (witness-folded twisted ElGamal well-formedness over any number of same-key ciphertexts at a constant `2` points + `2` scalars; a single ciphertext is the batch-of-1 case, and the whole batch of per-transfer auditor encryptions is proven by one such proof). Both verifiers reduce to two shared Schnorr row-checkers and bind the statement bases into the Blake2b256 challenge transcript, so the same proof types are reusable across different DDH/ElGamal contexts.
...
- **client.ts**: `ContraClient` -- builds Move call transactions for register, wrap, unwrap, transfer, and account management. Since deposits no longer auto-register on chain, `wrap` (now async) and `transferBatch` fetch each receiver's state and, for a receiver with an `Account` but no `TokenAccount<T>`, prepend a `registerWithDefaultPk` call in the same PTB (permissionless-register tokens only); `registerWithDefaultPk({ receiver, tokenType })` is also exposed for pre-registering. Transfers attach per-transfer auditor data (`buildAuditorData`: two u32-limb handles per receiver + one batched `ElGamalNizk`) when `getAuditor(tokenType)` reports an enabled key. `newAccount({ defaultPk? })` sets the account's optional default key (omit to create without one; built as `Option<Element<G>>`), and `register` passes the token's key explicitly (`tokenAccount.publicKey`). Key rotation: `setDefaultPk` sets/clears the optional default key (O(1)), `rekeyToken` / `tryRekeyToken` re-key a token to an explicit `newTokenAccount.publicKey` (independent of the account key; the latter soft-fails), and `tryRekeyTokens({ rotations })` (the batched, soft-failing plural of `tryRekeyToken`: optimistic per-token merge + `tryRekeyToken` for every `(current, new)` pair in one PTB — each token re-keys from its own current key to its own paired new key; racing tokens just soft-fail and stay stale; it does not touch the account default key — use `setDefaultPk` for that). `getTokenKeys(address, tokenTypes?)` reports each token's current key (a `TokenKeyStatus[]`); `tokenTypes` is optional — omit it to enumerate every token on the account via its dynamic fields (`listDynamicFields`). An app uses it to see each token's current key, to detect which tokens a `tryRekeyTokens` left un-rekeyed (soft-failed — the token still reports its old key), and whether an old key is still in use (a token can only be re-keyed/decrypted with the key it currently reports, so the old key must be retained until no token still reports it).
- **auditor.ts**: `ContraAuditor` -- per-transfer auditor. Holds the token's auditor private key and `decryptTransferAmount(encryptedAmountReceiver, auditorHandles)` recovers a transfer's amount from a `TransferEvent`: it regroups the receiver's four u16 limbs into the two u32-limb commitments (mirroring `encrypted_amount::ciphertexts_as_u32_limbs`), pairs each with the matching handle, and BSGS-decrypts. The auditor never learns a user's viewing key.
- **token_account.ts**: Client-side representation of a user's token account state. Includes `decryptWithProof(ciphertext, table)` for the selective-disclosure flow: given any `Ciphertext` (e.g. a collapsed balance or a `TransferEvent`'s encrypted amount), it returns `{ value, proof }` -- a plaintext and a zero-knowledge proof of correct decryption verifiable with `ciphertext.verifyDecryption(pk, value, proof)`. Also `recoverSentAmount(encryptedAmount, point, batchIndex, table)` for a sender to recover its own outgoing batched-transfer amounts: the sender stores no per-transfer secret and instead re-derives the per-limb blindings from `seed = HKDF(sk * point)` (the `TransferEvent`'s `transfer_randomness_point`) and reads the value off the commitments (which equal the receiver's), with no sender-keyed decryption handle.
...
- **nizk.ts**: NIZK proof generation/verification mirroring `nizk.move` — `DdhNizk` (shared-witness DDH over base/image pairs; two-pair balance/equality case, five-pair re-key case) and `ElGamalNizk` (witness-folded batch over same-key ciphertexts at constant size; `helpers.ts`'s `buildWellFormedProof` folds each amount's four limbs into one proof; the batch of per-transfer auditor encryptions is proven by one `ElGamalNizk`).
