...
- **balance.move**: `EncryptedBalance<T>` — an account's confidential balance (an `EncryptedAmount` plus a count of merged u16-bounded values that bounds limb growth) — and the linear coins that move value in/out of it, `PublicCoin<T>` and `EncryptedCoin<T>`. All are `phantom`-parameterized by the token type, mirroring `sui::balance::Balance<T>` / `sui::coin::Coin<T>`, so different token types can't be mixed. The balance has only `store` (no `copy`/`drop`) and is mutated in place: split (`try_split_to_public` / `try_split_batch`), merge (`merge_public` / `merge_encrypted` / `merge`), verified re-state (`try_update` / `set_public_key`), and `TreasuryCap`-gated issuer overrides (`overwrite_unchecked` / `clear_unchecked`). `try_split_batch` splits receiver-keyed `EncryptedCoin`s off the balance: it takes the receivers' and the new balance's `InRangeVerifiedEncryptedAmount`s and the transfer total as a `VerifiedEncryption` (verified upstream in `verify_transfer_amounts`). The total's commitment equals the sum of the receiver commitments (sender and receiver commitments are identical) **by construction** upstream — `verify_transfer_amounts` builds `total_sender` from that sum via `encrypted_amount::sum_ciphertexts` — so `try_split_batch` relies on its caller for that equality (per its doc) rather than re-checking, and a DDH balance proof shows the balance drops by exactly that total. The sender therefore never sends per-limb sender-keyed handles — only the single folded total handle.
