...
- **nizk.ts**: NIZK proof generation/verification mirroring `nizk.move` — `DdhNizk` (shared-witness DDH over base/image pairs; two-pair balance/equality case, five-pair re-key case, two-pair per-transfer auditor case — `client.ts`'s `buildAuditorData` builds one per (receiver, u32-limb)) and `ElGamalNizk` (witness-folded batch over same-key ciphertexts at constant size; `helpers.ts`'s `buildWellFormedProof` folds each amount's four limbs into one proof).
