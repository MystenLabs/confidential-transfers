...
- **contra.move**: Main contract — `TokenRegistry`, `AccountRegistry`, `Account`, `ConfidentialToken<T>`, `ManagementCap<T>`. Orchestrates register, wrap (public→confidential), unwrap (confidential→public), and transfer operations. Supports a permissioned mode where `register`/`wrap`/`unwrap` can be gated behind an issuer policy. Uses Sui dynamic fields for account state. Each token's balance is keyed under a per-token `TokenAccount.pk`, chosen explicitly at `register<T>(account, auth, pk)` and rotated explicitly via `rekey_token<T>(account, auth, new_pk, ...)` — per-token keys are independent of each other and of the account key. `Account.pk` is an **optional** default key (`Option<Element<G>>`): the only thing it drives is `register_permissionless`. `set_default_key` sets or clears it (`Option`, O(1), touches no balance). Key rotation is lazy: `rekey_token<T>` requires the token's `pending` empty (merge first) and re-keys `active` from `TokenAccount.pk` to the supplied `new_pk` under a per-token DDH (`balance::try_set_public_key`); the optimistic sibling `try_rekey_token<T>` soft-fails (emits `TryTokenRekeyFailedEvent`, returns `false`, leaves the token stale) instead of aborting, so `set_default_key` + several re-keys can ride in one PTB without pausing. Deposits land under the receiver's `TokenAccount.pk` (senders read the per-token key), so `active`/`pending` never mix keys and a stale token keeps receiving under its old key until re-keyed. Deposits never auto-register: `wrap` and `add_to_batch` abort with `EReceiverNotRegistered` if the receiver has no `TokenAccount<T>`. For a permissionless-register token whose owner set `Account.pk`, anyone can create one on the receiver's behalf up front with `register_permissionless<T>(account, ct)` (keyed at `Account.pk`; aborts `EDefaultKeyNotSet` if unset, `ERegistrationNotPermissionless` if the token's `register` op is permissioned) — the SDK inserts this automatically before a `wrap`/`transfer` to an as-yet-unregistered receiver. Deposits (`wrap` / `add_to_batch`) are gated on the per-`TokenAccount` `accepts_deposits`. Auditing is **per-transfer**: when a token has an auditor key, `batched_transfer` attaches two u32-limb auditor decryption handles per receiver plus one batched `ElGamalProof`, checked in `verify_auditing` against the current or (in grace) previous auditor key at `ctx.epoch()`.
- **auditors.move**: `Auditor` — a token's single auditor public key with a rotation grace window (`current_pk`, `previous_pk`, `previous_expiration_epoch`). `verify_transfer` accepts a per-transfer auditor `ElGamalProof` under the current key, or the previous key while `epoch <= previous_expiration_epoch`. Auditor data is required when a current key is set, forbidden when fully off (no current key and no previous key in grace), and optional during a disable grace window (`current_pk == none` but the previous key is still within `previous_expiration_epoch`) — data, if attached, is verified under the previous key so in-flight transfers stay auditable (`accepts_at`).
...
- **client.ts**: `ContraClient` -- builds Move call transactions for register, wrap, unwrap, transfer, and account management. Since deposits no longer auto-register on chain, `wrap` (now async) and `transferBatch` fetch each receiver's state and, for a receiver with an `Account` but no `TokenAccount<T>`, prepend a `registerPermissionless` call in the same PTB (permissionless-register tokens only); `registerPermissionless({ receiver, tokenType })` is also exposed for pre-registering. Transfers attach per-transfer auditor data (`buildAuditorData`: two u32-limb handles per receiver + one batched `ElGamalNizk`) when `getAuditor(tokenType)` reports an enabled key. `newAccount({ defaultKey? })` sets the account's optional default key (omit to create without one; built as `Option<Element<G>>`), and `register` passes the token's key explicitly (`tokenAccount.publicKey`). Key rotation: `setDefaultKey` sets/clears the optional default key (O(1)), `rekeyToken` / `tryRekeyToken` re-key a token to an explicit `newTokenAccount.publicKey` (independent of the account key; the latter soft-fails), and `tryRekeyTokens({ rotations })` (the batched, soft-failing plural of `tryRekeyToken`: optimistic per-token merge + `tryRekeyToken` for every `(current, new)` pair in one PTB — each token re-keys from its own current key to its own paired new key; racing tokens just soft-fail and stay stale; it does not touch the account default key — use `setDefaultKey` for that). `getTokenKeys(address, tokenTypes?)` reports the account's optional default key plus each token's current key and whether it is stale (differs from the default key, when set); `tokenTypes` is optional — omit it to enumerate every token on the account via its dynamic fields (`listDynamicFields`). An app uses it to see each token's current key, to detect which tokens a `tryRekeyTokens` left un-rekeyed (soft-failed — the token still reports its old key), and whether an old key is still in use (a stale token can only be re-keyed/decrypted with the key it currently reports, so the old key must be retained until it's caught up).
