...
- **auditor.ts**: `ContraAuditor` -- per-transfer auditor. Holds one or more auditor private keys (keyed by pk-bytes in a `Map`; `addKey` adds more; `publicKeys` lists them) and `decryptTransferAmount(event)` recovers a transfer's amount from a decoded `TransferEvent` (returns `null` when the transfer carried no auditor data): it uses the held key whose public key matches the event's `auditor_pk` and its handle set (`auditor_decryption_handle`), takes the two u32-limb commitments straight from the event's `encrypted_amount_receiver` (two u32 `Encryption`s, no regrouping), pairs each with the matching handle, and BSGS-decrypts. A token has at most one auditor key at a time, but holding the rotated-out keys lets one auditor read transfers made before and after a key rotation. The auditor never learns a user's viewing key.
