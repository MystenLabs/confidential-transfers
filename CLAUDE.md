...
- **core/**: wire types byte-compatible with `contra::guardian`, the plaintext arithmetic checks, and approval signing. No Nitro, no networking, so it is unit-testable off-enclave. Values are fastcrypto types (`RistrettoPoint`, `Ciphertext`, `Ed25519PublicKey`); only the fields the chain rebuilds carry a `move_*` serde adapter, because Move's `Element<G>` BCS-encodes with a length prefix that fastcrypto's point encoding omits. `tests` in `types.rs` pin those bytes against the Move golden vectors.
