...
- **guardian.move**: Optional TEE second factor for transfers (design: `GUARDIAN.md`). `GuardianPolicy` (held as `Option<GuardianPolicy>` on `ConfidentialToken` — `Some` enables it) tracks the expected enclave image PCRs, a `version`/`min_version` pair for lazy key invalidation, a single operator address, an operator-updatable discovery `url`, and the registered `GuardianEnclaveKey`s (VecMap keyed by ed25519 `signing_pk`). Registration verifies a `sui::nitro_attestation` document against the policy PCRs and takes both keys from its `user_data`. When enabled, `batched_transfer` and `unwrap` require a `GuardianApproval`: an enclave signature over the BCS `TransferApprovalPayload` / `UnwrapApprovalPayload`. Issuer wrappers (`ManagementCap`) and the operator entry points live in `contra.move`; events in `events.move`.
