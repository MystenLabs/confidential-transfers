...
- **nizk.ts**: NIZK proof generation/verification mirroring `nizk.move` — `DdhNizk` (shared-witness DDH over base/image pairs; two-pair balance/equality case, five-pair re-key case, `(1+M)`-pair per-transfer auditor case — and `DdhNizk.proveBatch` folds a receiver's two u32 limbs (shared bases, independent witnesses) into one proof, so `client.ts`'s `buildAuditorData` builds one per receiver) and `ElGamalNizk` (witness-folded batch over same-key ciphertexts at constant size; `helpers.ts`'s `buildWellFormedProof` folds each amount's four limbs into one proof).
