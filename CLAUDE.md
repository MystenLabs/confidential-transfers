...
- **contra.move**: Main contract — `TokenRegistry`, `AccountRegistry`, `Account`, `ConfidentialToken<T>`, `ManagementCap<T>`. Orchestrates register, wrap (public→confidential), unwrap (confidential→public), and transfer operations. Supports a permissioned mode where `register`/`wrap`/`unwrap` can be gated behind an issuer policy. Uses Sui dynamic fields for account state. The user's ElGamal public key is account-level (`Account.pk`), shared by every token balance; rotation is whole-account via a `KeyRotation` hot-potato state machine and a two-phase commit (`begin_key_rotation` → one `stage_token_rekey<T>` per token appending its four old/new limb-handle pairs to the receipt and staging its re-keyed amount → `finish_staging` verifying **one** batched re-keying `DdhProof` over all `4m+1` accumulated pairs (the account key plus every limb handle of all `m` tokens, one shared witness `w = sk'·sk⁻¹`) → one `finalize_token_rekey<T>` per token popping + committing its staged amount → `try_finish_key_rotation_and_unpause`, which flips the key once `staged` is drained); the `Staging`/`Finalizing`/`Failed` variants make the phase a type invariant. A raced deposit makes that batched proof fail at `finish_staging`, flipping the receipt to `Failed` so the rotation soft-fails and the account is left paused + merged for a race-free retry. The caller pauses the account-wide `accepts_deposits` gate (`set_account_accepts_deposits(false)`) before rotating (not enforced); `try_finish_key_rotation_and_unpause` resumes it on success. Deposits (`wrap` / `add_to_batch`) are accepted only if both that gate and the per-`TokenAccount` gate allow it. Auditing is **per-transfer**: when a token has an auditor key, `batched_transfer` attaches two u32-limb auditor decryption handles per receiver plus one batched `ElGamalProof`, checked in `verify_auditing` against the current or (in grace) previous auditor key at `ctx.epoch()`.
