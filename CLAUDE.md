...
- **client.ts**: `ContraClient` -- builds Move call transactions for register, wrap, unwrap, transfer, and account management. Since deposits no longer auto-register on chain, `wrap` (now async) and `transferBatch` fetch each receiver's state and, for a receiver with an `Account` but no `TokenAccount<T>`, prepend a `registerPermissionless` call in the same PTB (permissionless-register tokens only); `registerPermissionless({ receiver, tokenType })` is also exposed for pre-registering. Transfers attach per-transfer auditor data (`buildAuditorData`: two u32-limb handles per receiver + one batched `ElGamalNizk`) when `getAuditor(tokenType)` reports an enabled key. Key rotation follows the Move Option A model: `setAccountKey` (O(1)), `rekeyToken` / `tryRekeyToken` (per token; the latter soft-fails), the `rotateKey` convenience (set-key + merge + re-key one token in one PTB), and `rotateKeyAll` (set-key once + optimistic per-token merge + `tryRekeyToken` for every listed token in one PTB — racing tokens just soft-fail and stay stale). `getTokenKeys(address, tokenTypes?)` reports the account key plus each token's current key and whether it is stale (lagging the account key); `tokenTypes` is optional — omit it to enumerate every token on the account via its dynamic fields (`listDynamicFields`). An app uses it to see what to re-key, to detect which tokens a `rotateKeyAll` left stale (soft-failed), and whether an old key is still in use (a stale token can only be re-keyed/decrypted with the key it currently reports, so the old key must be retained until it's caught up).
