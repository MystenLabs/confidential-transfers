...
- **client.ts**: `ContraClient` -- builds Move call transactions for register, wrap, unwrap, transfer, and account management. Transfers attach per-transfer auditor data (`buildAuditorData`: two u32-limb handles per receiver + one batched `ElGamalNizk`) when `getAuditor(tokenType)` reports an enabled key. Key rotation follows the Move Option A model: `setAccountKey` (O(1)), `rekeyToken` / `tryRekeyToken` (per token; the latter soft-fails), and the `rotateKey` convenience (set-key + merge + re-key in one PTB). `getTokenKeys(address, tokenTypes)` reports the account key plus each token's current key and whether it is stale (lagging the account key) — so an app can see what to re-key and whether an old key is still in use (a stale token can only be re-keyed/decrypted with the key it currently reports, so the old key must be retained until it's caught up).
